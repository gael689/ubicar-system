...
class PagareService:
    ...
    def faltantes(self, conf: dict | None = None) -> list[str]:
        ...
        if not conf["interes_compensatorio"]:
            faltan.append("la tasa de interés compensatorio (Configuración → Pagaré)")
        if not conf["interes_punitorio"]:
            faltan.append("la tasa de interés punitorio (Configuración → Pagaré)")
    ...
    def get(self, pagare_id: int) -> Pagare:
        ...
        if not p:
            raise NotFoundError("Pagaré", pagare_id)
        return p
    ...
    def _limpiar_codeudores(codeudores: list[dict] | None) -> list[dict]:
        ...
        if len(limpios) > MAX_CODEUDORES:
            raise BusinessRuleError(
                "demasiados_codeudores", f"El pagaré admite hasta {MAX_CODEUDORES} co-deudores."
            )
        return limpios
    ...
    def crear(
        ...
        *,
        ...
        if contrato is None:
            raise BusinessRuleError(
                "pagare_sin_contrato",
                "Generá el contrato primero: el pagaré viaja en el mismo link y se "
        ...
        if existente is not None:
            raise BusinessRuleError(
                "pagare_ya_existe",
                f"La reserva ya tiene el pagaré {existente.numero_formateado}. "
        ...
        try:
            ...
            raise BusinessRuleError("pagare_monto_invalido", "El monto del pagaré no es un número.")
        if monto_dec <= 0:
            raise BusinessRuleError("pagare_monto_invalido", "El monto del pagaré tiene que ser mayor a cero.")
        ...
        if faltan:
            raise BusinessRuleError(
                ...
                "Para emitir el pagaré falta cargar " + "; ".join(faltan) + ".",
            )
        ...
        auditoria_service.registrar(
            ...
            descripcion=(
                f"Pagaré {pagare.numero_formateado} por ${snapshot['monto_numerico']} "
    ...
    def validar_firma(self, pagare: Pagare, medio: str, codeudores: list[dict] | None) -> None:
        ...
        """
        if pagare.anulado:
            raise BusinessRuleError("pagare_anulado", "El pagaré está anulado.")
        if pagare.firmado:
            raise BusinessRuleError("pagare_ya_firmado", "El pagaré ya está firmado.")
    ...
    def anular(self, pagare_id: int, motivo: str, usuario_id: int | None) -> Pagare:
        ...
        if pagare.anulado:
            raise BusinessRuleError("pagare_ya_anulado", "El pagaré ya está anulado.")
        ...
        auditoria_service.registrar(
            ...
            entidad_id=pagare.id,
            descripcion=(
                f"Pagaré {pagare.numero_formateado} anulado"
                + (" — estaba FIRMADO" if estaba_firmado else "")
                + f". Motivo: {motivo}"
            ),
    ...
    def adjuntar_escaneo(self, pagare_id: int, contenido: bytes, content_type: str) -> Pagare:
        ...
        if extension is None:
            raise BusinessRuleError(
                "formato_no_soportado", "Subí el pagaré firmado como PDF, JPG, PNG o WEBP."
            )
