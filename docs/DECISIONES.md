...
| **D-54** ✅ | **Upgrade a categoría superior al mismo precio, siempre propuesto.** Nunca automático. Es herramienta del mostrador, no del flujo web | Implementado el tracking: `reservas.categoria_entregada_id` / `upgrade_motivo` se completan al asignar un vehículo de otra categoría. **Sigue abierto** (§5, pregunta 2): por qué canal se lo propone al cliente y quién registra que lo aceptó — no bloquea, es el siguiente paso de UI sobre esta base |
...
| **D-56** 🔄 | Los lugares salen de Configuración; "Otro" deriva a un vendedor | **Cierra D-10** en la mitad que se implementó: `web.lugares_retiro` en `configuracion` es ahora la única fuente — se sacaron las tres listas hardcodeadas que había (Hero, buscador del flujo, ambas con textos ligeramente distintos). "Otro lugar" sigue resuelto como texto libre coordinado por WhatsApp (ya existía), **todavía no** por el cartel único de D-59 — queda para una vuelta siguiente |
