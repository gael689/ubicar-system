...
| **D-58** ✅ | **La home muestra lo que está trabado, y suma una vista anual.** Barra al pie sólo cuando hay reservas sin asignar, desplegable en el lugar; y una tercera pestaña con el año completo | **Enmienda D-24.** Implementado: panel "Pendiente de asignación" en `/ocupacion` (sólo ocupa lugar si hay algo pendiente) y una vista anual navegable a la vista timeline. **Enmendado por D-62 (14/08):** ya no pinta por densidad de ocupación — pinta por estado de reserva, con los mismos colores del timeline |
...
| **D-62** ✅ | **El calendario anual pinta por estado de reserva, no por densidad.** Los mismos siete colores del timeline, y es la vista principal de ocupación | **Enmienda D-58.** El degradé de azul por ocupación no se podía leer contra ninguna leyenda: "muy ocupado / ocupación media" no explicaba nada al que venía de ver barras de colores por estado. Ahora el fondo del día es **el estado más urgente** (`vencida > activa > confirmada > pendiente > bloqueo > finalizada`) y, si ese día hay además otros estados, una franja al pie con un segmento por cada uno — un día con una vencida sola no se ve igual que uno con una vencida entre diez activas. **Prioridad y no mayoría**: la vista anual sirve para decidir qué día hay que mirar. Los colores se derivan de `ESTADO_COLORS_EVENTO`, la misma tabla del timeline, filtrando los tokens que a 45px son ruido (`animate-pulse` con 40 celdas rojas latiendo, `line-through`, `border-dashed`) — no hay una segunda paleta que se pueda desincronizar. **`cancelada` no se pinta**, y no es una omisión de esta vista: la consulta que alimenta el timeline tampoco la trae, y la anual es una pre-vista suya. Los bloqueos ahora sí cuentan en el total de ocupados: un auto en el taller tampoco está disponible |

| **D-63** ⏳ | **Tres niveles de cobertura, y la franquicia sale de la categoría** | **Decidido por Gael el 14/08, sin implementar.** Reemplaza los números de referencia de D-53. **LDW** es la base (resp. civil, robo, incendio y daños): Compacto y Sedán $1.500.000 · Sedán superior $2.000.000 · Pick-up $3.000.000. **TOP COVER** = base − $500.000 y **+20%** al alquiler. **SUPER TOP COVER** = base − $1.000.000 y **+45%**. Los porcentajes **no son acumulativos**: van sobre el alquiler base, así que Super Top es +45% y nunca +20% y después +45%. **Ninguna de las tres cubre ruedas y vidrios** — es un servicio aparte que cuesta **10% más por día**: se toma el precio total, se divide por la cantidad de días, y a ese precio diario se le suma el 10%. *(Aritméticamente da lo mismo que 10% del total; se calcula y se muestra por día porque es como se le explica al cliente.)* **SUV y Furgón todavía no tienen franquicia definida y, mientras no la tengan, no se reservan por la web**: caen en el panel de derivación a un agente (D-61) |
| **D-64** ⏳ | **En un upgrade rige la franquicia del vehículo ENTREGADO, no la del contratado** | **Decidido por Gael el 14/08, sin implementar.** Si alguien contrata Compacto y se le entrega una Pick-up al mismo precio (D-54), el contrato dice la franquicia de la Pick-up. **Ubicar no absorbe la diferencia.** El fundamento es operativo y cierra el caso: **el contrato se firma al momento de entregar el vehículo**, con upgrade o sin él, así que cuando el cliente firma ya sabe qué auto se lleva y con qué franquicia — no hay sorpresa posterior. Y si hubo upgrade fue porque se le ofreció otra cosa y aceptó. **Esto corrige el comportamiento actual**: `contrato_service` lee hoy `r.categoria` (la contratada), que además lo hace sin decirlo y sin ningún test — tiene que leer `categoria_entregada_id` cuando existe |
| **D-65** ⏳ | **Al cambiar o reasignar el vehículo se puede corregir el precio** | **Decidido por Gael el 14/08, sin implementar.** El panel de reasignación tiene que dejar modificar el precio **por día y también el total**. Sin esto, un cambio de vehículo obliga a elegir entre entregar el auto que hay o respetar el precio pactado. Va junto con la reasignación, y con el mismo criterio que el resto de las decisiones de plata del sistema: quien cambia un precio a mano deja motivo, y queda auditado |
