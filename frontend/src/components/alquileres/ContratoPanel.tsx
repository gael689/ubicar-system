      ...
      <Card className="p-5 space-y-4">
                ...
                onSuccess: () => {
                  ...
                  crearPagare.mutate(
                    { reserva_id: reservaId, monto: parseFloat(dp.monto), codeudores: dp.codeudores },
                    {
                      ...
                      onError: e => toast.error(`El contrato se generó, pero la franquicia no: ${extractError(e)}`),
                    },
                  );
                },
              },
        ...
        >
          {crear.isPending || crearPagare.isPending
            ? 'Generando…'
            : conPagare && pagarePreparado?.faltantes.length === 0 ? 'Generar contrato y franquicia' : 'Generar contrato'}
        </Button>
      </Card>
    );
  }
...
function BloqueFirma({ contrato, pagare }: { contrato: Contrato; pagare: Pagare | null }) {
  ...
  const mensajeWhatsapp = link
    ? link.mensaje || (pagare
      ? `Te paso ${contrato.firmado ? 'la franquicia para que la leas y la firmes' : 'el contrato de alquiler y la franquicia para que los leas y los firmes'} desde el celular:\n\n${link.url}`
    ...
    <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-foreground">
          {pagare && contrato.firmado ? 'Que firme la franquicia' : pagare ? 'Que firme el contrato y la franquicia' : 'Que lo firme el cliente'}
        </p>
      </div>
      ...
      {!link ? (
        <>
          <p className="text-xs text-muted-foreground">
            ...
            {pagare && ' En el mismo link, abajo del contrato, está la franquicia: una sola firma vale para los dos.'}
            {' '}Cuando firma,{' '}
...
function FirmaDialog({
  ...
  const titulo = firmaContrato && pagare ? 'Firmar contrato y franquicia' : pagare ? 'Firmar la franquicia' : 'Firmar contrato';
      ...
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-xl bg-background p-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            ...
            {pagare && firmaContrato && ' La misma firma queda en el contrato y en la franquicia.'}
          </p>
        </div>
        ...
        {medio === 'papel' ? (
          <p className="rounded-lg bg-muted px-3 py-2.5 text-xs text-muted-foreground">
            ...
            que archivarlo{pagare ? ' (el de la franquicia, con más razón: es el que se presenta al cobro)' : ''}.
